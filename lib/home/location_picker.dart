...
class _LocationPickerSheetState extends State<_LocationPickerSheet> {
  ...
  Future<void> _search(String query) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final results = (body['results'] as List?) ?? [];
        setState(() {
          _searchResults = results.take(6).map<Map<String, dynamic>>((item) {
  ...
  Future<void> _useGps() async {
    ...
    try {
      ...
      String address = await reverseGeocodeAddress(pos.latitude, pos.longitude);
      if (!mounted) return;
              ...
              child: ListView(
                ...
                children: [
                  ...
                  if (_searchResults.isNotEmpty) ...[
                        ...
                        onTap: () {
                          ...
                          if (!_isInSekondiTakoradi(lat, lng)) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                content: Text(
                                  'WastePick is currently only available in '
                    ...
                    ...saved.map(
                      (s) {
                        final lat = (s['lat'] as double?) ?? provider.customerLocation.latitude;
                        final lng = (s['lng'] as double?) ?? provider.customerLocation.longitude;
